...
<script type="text/javascript">
	...
	function openIncomeModifyDialog() {
		...
		$('#fm').form('load', row);
		url = "${basePath}incomesave.do?id=" + row.id;
	}
...
<body style="margin: 1px;">
	<table id="dg" title="收入管理" class="easyui-datagrid" fitColumns="true"
		pagination="true" rownumbers="true" url="${basePath}incomelist.do?roleid=1&userid=1"
		...
		<div>
			...
			&nbsp;收入类型：&nbsp;<select class="easyui-combobox" id="s_dataid" editable="false" style="width: 100px;">
				...
				<c:forEach items="${incomes }" var="income">
					<option value="${income.id }">${income.datadicvalue }</option>
				</c:forEach>
			</select>&nbsp;
