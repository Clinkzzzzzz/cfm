...
<script>
	function searchLine(){
		...
		$.post("${basePath}produceIncomeTime.do", {incomer : s_incomer,starttime:s_starttime,endtime:s_endtime,roleid:"1",userid:"1"}, function(result) {
			//roleid:"${currentUser.roleid}",userid:"${currentUser.id}"
		    ...
		    for(var i=0;i<result.length;i++){
		    	...
		    	$("#containerPie"+i).highcharts({
			        ...
			        title: {
			            text: result[i].name+'：(时间——金额)收入饼状图'
			        },
			        tooltip: {
			            pointFormat: '<b>{series.name}</b><br/>金额：<b>{point.y}元</b>'
			        },
...
<body>
	<div style="padding:5px;">
		&nbsp;收入用户：&nbsp;<input type="text" id="s_incomer" size="15" onkeydown="if(event.keyCode==13) searchLine()" />
		&nbsp;收入起止时间：&nbsp;<input type="text" id="s_starttime" class="easyui-datetimebox" size="18" onkeydown="if(event.keyCode==13) searchLine()"/>
